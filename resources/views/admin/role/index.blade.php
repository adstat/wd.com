				...
				<div class="layui-col-md12">
					<div class="layui-card" style="padding:15px;">
						<div class="display conter" id="flgl" style="margin-bottom: 20px;">
							...
							<a href="{{url('admin/role/create')}}" data-method="notice" id="daoru" style="background: #3074b7;">添加</a>
						</div>
...
<script>
    ...
    function GetDataCount() {
        ...
        var url = '{{ url('admin/role/getRoleCount') }}';
    ...
    function getPage(pageSize,page) {
        ...
        var tr='';
        $.ajax({
            url: '{{url('admin/role/getRoleList')}}',
            dateType:'json',
            type:'post',
            async:true,
            cache:false,
            ...
            success:function(res){
                ...
                $.each(list,function (k, v) {
                    ...
                    tr+='<td>'+v.role_id+'</td>';
                    tr+='<td>'+v.role_name+'</td>';
                    tr+='<td>';
                    tr+='<a  onclick="updateOne('+v.role_id+');" class="bianji">编辑</a>';
                    tr+='<span onclick="deleteOne('+v.role_id+');" class="refuse">删除</span>';
                    tr+='</td>';
                    tr+='</tr>';
                });
                $('#show_admin').html(tr);
    ...
    function deleteOne(id){
        ...
        if (confirm('确定要删除吗？')) {
            var url='{{url('admin/role/deleteOne')}}';
            var data={'id':id};
            $.post(url,data , function (res) {
                if (res) {
                    alert('删除成功！');
