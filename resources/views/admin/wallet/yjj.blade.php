...
<script>
    ...
    function GetDataCount() {
        ...
        var data = {search : searchName,statu:4};
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                $.each(list,function (k, v) {
                    ...
                    tr+='<td>'+v.time+'</td>';
                    tr+='<td>';
                    tr+='<span onclick="deleteOne('+v.wallet_inv+');" class="refuse">删除</span>';
                    tr+='</td>';
                    tr+='</tr>';
                });
                $('#show_ddk').html(tr);
    ...
    function deleteOne(wallet_inv){
        if (confirm('确定要删除吗？')) {
            var url = '{{ url('admin/wallet/getDeleteOne') }}';
            var data = {wallet_inv : wallet_inv};
            $.post(url,data , function (res) {
                if (res) {
                    alert('删除成功！');
                    GetDataCount();
