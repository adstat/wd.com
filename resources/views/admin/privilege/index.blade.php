				...
				<div class="layui-col-md12">
					<div class="layui-card" style="padding:15px;">
						<div class="display conter" id="flgl" style="margin-bottom: 20px;">
							...
							<a href="{{url('admin/privilege/create')}}" data-method="notice" id="daoru" style="background: #3074b7;">添加</a>
						</div>
...
<script>
    ...
    function GetDataCount() {
        ...
        var url = '{{ url('admin/privilege/getPrivilegeCount') }}';
    ...
    function getPage(pageSize,page) {
        ...
        var tr='';
        $.ajax({
            url: '{{url('admin/privilege/getPrivilegeList')}}',
            dateType:'json',
            type:'post',
            async:true,
            cache:false,
            ...
            success:function(res){
                ...
                $.each(list,function (k, v) {
                    ...
                    tr+='<td>'+v.pri_id+'</td>';
                    tr+='<td>'+v.pri_name+'</td>';
                    tr+='<td>'+v.controller_name+'</td>';
                    tr+='<td>'+v.module_name+'</td>';
                    tr+='<td>'+v.action_name+'</td>';
                    tr+='<td>'+v.parent_id+'</td>';
                    tr+='<td>';
                    tr+='<a  onclick="updateOne('+v.pri_id+');" class="bianji">编辑</a>';
                    tr+='<span onclick="deleteOne('+v.pri_id+');" class="refuse">删除</span>';
                    tr+='</td>';
                    tr+='</tr>';
                });
                $('#show_admin').html(tr);
    ...
    function deleteOne(id){
        ...
        if (confirm('确定要删除吗？')) {
            var url='{{url('admin/privilege/deleteOne')}}';
            var data={'id':id};
            $.post(url,data , function (res) {
                if (res) {
                    alert('删除成功！');
